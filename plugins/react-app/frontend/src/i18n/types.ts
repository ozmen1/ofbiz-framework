export type Locale = 'tr' | 'en';

export interface Translations {
  common: {
    save: string;
    cancel: string;
    delete: string;
    edit: string;
    back: string;
    create: string;
    search: string;
    filter: string;
    refresh: string;
    actions: string;
    status: string;
    date: string;
    dueDate: string;
    amount: string;
    loading: string;
    noData: string;
    total: string;
    details: string;
    all: string;
    yes: string;
    no: string;
    close: string;
    success: string;
    error: string;
    currency: string;
    description: string;
    party: string;
    reference: string;
    type: string;
    confirm: string;
    selected: string;
    reset: string;
    previous: string;
    next: string;
    open: string;
    applied: string;
    active: string;
    inactive: string;
    assigned: string;
    unassigned: string;
    print: string;
    downloadCsv: string;
    viewIndex: string;
    of: string;
    page: string;
    select: string;
    none: string;
    theme: string;
    lightMode: string;
    darkMode: string;
    copyLink: string;
    linkCopied: string;
    id: string;
    saving: string;
  };
  nav: {
    appName: string;
    appSubtitle: string;
    mainMenu: string;
    accounting: string;
    generalLedger: string;
    advancedAccounting: string;
    system: string;
    modules: string;
    accountingOverview: string;
    operations: string;
    orderManagement: string;
    salesOrders: string;
    purchaseOrders: string;
    orderQuotes: string;
    orderReturns: string;
    shipments: string;
    salesCycle: string;
    purchasingAndReturns: string;
    overview: string;
    manufacturing: string;
    productionRuns: string;
    billOfMaterials: string;
    routings: string;
    inventory: string;
    facilities: string;
    inventoryTransfers: string;
    physicalInventory: string;
    comingSoon: string;
    planned: string;
    activeModule: string;
    dashboard: string;
    invoices: string;
    payments: string;
    paymentGroups: string;
    cashAndBank: string;
    chartOfAccounts: string;
    journalEntries: string;
    fiscalPeriods: string;
    reports: string;
    assetsAndBudget: string;
    taxAndGl: string;
    fxRates: string;
    costCenters: string;
    depositSlips: string;
    accountingPreferences: string;
    paymentGateways: string;
    checkRun: string;
    commissionRun: string;
    apiTest: string;
    settings: string;
    logout: string;
    newInvoice: string;
    newPayment: string;
    parties: string;
    users: string;
    systemAdmin: string;
    catalogAndProduct: string;
    products: string;
    catalogs: string;
    categories: string;
    variantsAndAssocs: string;
    pricingAndPromos: string;
    stores: string;
    advancedInventory: string;
    configItems: string;
  };
  pages: {
    dashboard: { title: string; subtitle: string };
    invoices: { title: string; subtitle: string };
    createInvoice: { title: string; subtitle: string };
    invoiceDetail: { title: string; subtitle: string };
    payments: { title: string; subtitle: string };
    createPayment: { title: string; subtitle: string };
    paymentDetail: { title: string; subtitle: string };
    paymentGroups: { title: string; subtitle: string };
    financialAccounts: { title: string; subtitle: string };
    chartOfAccounts: { title: string; subtitle: string };
    journalEntries: { title: string; subtitle: string };
    createJournalEntry: { title: string; subtitle: string };
    fiscalPeriods: { title: string; subtitle: string };
    reports: { title: string; subtitle: string };
    advancedAccounting: { title: string; subtitle: string };
    taxAndGlMapping: { title: string; subtitle: string };
    fxRates: { title: string; subtitle: string };
    costCenters: { title: string; subtitle: string };
    depositSlips: { title: string; subtitle: string };
    accountingPreferences: { title: string; subtitle: string };
    paymentGateways: { title: string; subtitle: string };
    checkRun: { title: string; subtitle: string };
    commissionRun: { title: string; subtitle: string };
    testPage: { title: string; subtitle: string };
    orders: { title: string; subtitle: string };
    salesOrders: { title: string; subtitle: string };
    purchaseOrders: { title: string; subtitle: string };
    orderQuotes: { title: string; subtitle: string };
    orderReturns: { title: string; subtitle: string };
    shipments: { title: string; subtitle: string };
    manufacturing: { title: string; subtitle: string };
    inventory: { title: string; subtitle: string };
    parties: { title: string; subtitle: string };
    users: { title: string; subtitle: string };
    systemAdmin: { title: string; subtitle: string };
    products: { title: string; subtitle: string };
    catalogs: { title: string; subtitle: string };
    categories: { title: string; subtitle: string };
    variantsAndAssocs: { title: string; subtitle: string };
    pricingAndPromos: { title: string; subtitle: string };
    stores: { title: string; subtitle: string };
    advancedInventory: { title: string; subtitle: string };
    configItems: { title: string; subtitle: string };
  };
  dashboard: {
    totalRevenue: string;
    netReceivables: string;
    openPayables: string;
    pendingInvoices: string;
    recentInvoices: string;
    recentPayments: string;
    quickActions: string;
    viewAllInvoices: string;
    viewAllPayments: string;
    trendUp: string;
    trendDown: string;
    noRecentInvoices: string;
    activeCustomers: string;
    totalPayments: string;
    systemStatus: string;
    systemOnline: string;
    ofbizCore: string;
    delegator: string;
    activeUsers: string;
    securityGroups: string;
    scheduledJobs: string;
    jvmMemory: string;
    moduleLaunchers: string;
    accountingModuleDesc: string;
    partiesModuleDesc: string;
    usersModuleDesc: string;
    systemAdminModuleDesc: string;
    plannedModules: string;
    plannedModulesDesc: string;
    systemHealth: string;
    viewSystemAdmin: string;
    viewUsers: string;
    viewParties: string;
    refreshDashboard: string;
    coreEngine: string;
    databaseEngine: string;
    activeJobsCount: string;
    heapMemoryUsed: string;
  };
  invoices: {
    invoiceId: string;
    type: string;
    partyFrom: string;
    partyTo: string;
    invoiceDate: string;
    dueDate: string;
    totalAmount: string;
    paidAmount: string;
    outstandingAmount: string;
    status: string;
    newInvoice: string;
    searchPlaceholder: string;
    allTypes: string;
    allStatuses: string;
    salesInvoice: string;
    purchaseInvoice: string;
    customerRefund: string;
    itemsCount: string;
    addItem: string;
    itemType: string;
    quantity: string;
    unitPrice: string;
    lineTotal: string;
    notesAndTerms: string;
    statusInProcess: string;
    statusApproved: string;
    statusPaid: string;
    statusCancelled: string;
    statusReady: string;
    statusSent: string;
    copyInvoice: string;
    copyConfirm: string;
    deleteItemConfirm: string;
    statusChangeConfirm: string;
    subtotalWithoutTax: string;
    statusHistory: string;
    appliedPayments: string;
    itemsSubtext: string;
    noItems: string;
    noPaymentsApplied: string;
    noHistory: string;
    headerInfo: string;
    editHeader: string;
    invoiceCompleted: string;
    invoiceCancelledMsg: string;
    notFound: string;
    notFoundSub: string;
    backToList: string;
    descriptionPlaceholder: string;
    creating: string;
    itemDescOrName: string;
    kpiTotalInvoiced: string;
    kpiOutstanding: string;
    kpiPastDue: string;
    kpiPendingApproval: string;
    tabAll: string;
    tabSales: string;
    tabPurchase: string;
    tabPastDue: string;
    tabDueSoon: string;
    tabInProcess: string;
    exportCsv: string;
    exportSuccess: string;
    countUnit: string;
    printInvoice: string;
    printPreview: string;
    printAction: string;
    downloadPdf: string;
    generatingPdf: string;
    invoiceTitle: string;
    billFrom: string;
    billTo: string;
    grandTotal: string;
    balanceDue: string;
    authorizedStamp: string;
    recipientSignature: string;
    editItem: string;
    applyPayment: string;
    removeApplication: string;
    removeApplicationConfirm: string;
    openPayments: string;
    noOpenPayments: string;
    selectPayment: string;
    applyAmount: string;
    availableUnapplied: string;
    paymentApplicationSuccess: string;
    paymentApplicationRemoved: string;
    itemUpdated: string;
    rolesAndAttributesTitle: string;
    tabAddresses: string;
    tabAttributes: string;
    tabRoles: string;
    noAddresses: string;
    addAddress: string;
    selectRegisteredAddress: string;
    manualAddress: string;
    addressPurpose: string;
    addressDetail: string;
    unlinkAddress: string;
    unlinkAddressConfirm: string;
    addressLinked: string;
    addressUnlinked: string;
    noAttributes: string;
    addAttribute: string;
    attrName: string;
    attrValue: string;
    attrDescription: string;
    quickPresets: string;
    deleteAttrConfirm: string;
    attrSaved: string;
    attrDeleted: string;
    noRoles: string;
    addRole: string;
    roleParty: string;
    roleType: string;
    rolePercentage: string;
    deleteRoleConfirm: string;
    roleSaved: string;
    roleDeleted: string;
    tabAttachments: string;
    noAttachments: string;
    addAttachment: string;
    docName: string;
    docType: string;
    docDescription: string;
    deleteAttachmentConfirm: string;
    attachmentSaved: string;
    attachmentDeleted: string;
    sendEmailAction: string;
    sendEmailModalTitle: string;
    recipientEmail: string;
    ccEmail: string;
    emailSubject: string;
    emailBody: string;
    sendEmailSuccess: string;
    sendEmailBtn: string;
    sendingEmail: string;
  };
  payments: {
    paymentId: string;
    paymentType: string;
    fromParty: string;
    toParty: string;
    paymentMethod: string;
    effectiveDate: string;
    amount: string;
    newPayment: string;
    searchPlaceholder: string;
    allTypes: string;
    allStatuses: string;
    customerReceipt: string;
    vendorDisbursement: string;
    customerRefund: string;
    statusSent: string;
    statusReceived: string;
    statusCancelled: string;
    statusNotPaid: string;
    statusConfirmed: string;
    statusVoid: string;
    appliedAmount: string;
    unappliedAmount: string;
    applyToInvoice: string;
    receiptIn: string;
    disbursementOut: string;
    registeredPayments: string;
    registeredPaymentsSub: string;
    customerReceipts: string;
    customerReceiptsSub: string;
    vendorDisbursements: string;
    vendorDisbursementsSub: string;
    unappliedBalance: string;
    unappliedBalanceSub: string;
    allPaymentTypes: string;
    allPaymentMethods: string;
    allPaymentStatuses: string;
    resetFilters: string;
    appliedOpen: string;
    openAmountLabel: string;
    amountRequired: string;
    partiesRequired: string;
    partiesCannotBeSame: string;
    paymentCreateSuccess: string;
    paymentCreateError: string;
    paymentDetailsTitle: string;
    paymentDetailsSubtitle: string;
    senderPayer: string;
    receiverPayee: string;
    paymentAmount: string;
    paymentRef: string;
    notesDesc: string;
    selectPartyPrompt: string;
    paymentDetailsHeader: string;
    applyInvoice: string;
    appliedInvoices: string;
    applyPaymentTitle: string;
    selectInvoice: string;
    applyAll: string;
    matchAmount: string;
    confirmRemoveMember: string;
    confirmDeleteGroup: string;
    backToList: string;
    remainingDue: string;
    invoiceRemaining: string;
    paymentOpenBalance: string;
  };
  paymentGroups: {
    title: string;
    subtitle: string;
    newGroup: string;
    groupType: string;
    allTypes: string;
    totalGroups: string;
    totalPayments: string;
    totalAmount: string;
    groupName: string;
    createdAt: string;
    addPayment: string;
    memberPayments: string;
    noGroups: string;
    noPayments: string;
    groupNameRequired: string;
    selectAtLeastOnePayment: string;
    paymentsSelected: string;
    removeMember: string;
    deleteGroupConfirm: string;
    removeMemberConfirm: string;
    paymentCount: string;
  };
  chartOfAccounts: {
    title: string;
    subtitle: string;
    accountCode: string;
    accountName: string;
    accountClass: string;
    accountType: string;
    parentAccount: string;
    companyStatus: string;
    normalSide: string;
    debitBalance: string;
    creditBalance: string;
    totalDebits: string;
    totalCredits: string;
    newAccount: string;
    ledgerEntries: string;
    searchPlaceholder: string;
    allAccounts: string;
    assetAccounts: string;
    liabilityAccounts: string;
    equityAccounts: string;
    revenueAccounts: string;
    expenseAccounts: string;
    onlyAssignedToCompany: string;
    assignedActive: string;
    unassignedIdle: string;
    assignToggleTitle: string;
    inspect: string;
    registeredAccounts: string;
    assignedToCompany: string;
    activeUsedAccounts: string;
    normalSideDebit: string;
    normalSideCredit: string;
    useAsActiveInCompany: string;
    noJournalEntriesForAccount: string;
    editAccountTitle: string;
    summaryAndEntries: string;
    normalSideDirection: string;
    accountClassClass: string;
    accountTypeType: string;
  };
  journalEntries: {
    title: string;
    subtitle: string;
    transId: string;
    transDate: string;
    transType: string;
    reference: string;
    totalDebit: string;
    totalCredit: string;
    status: string;
    balanced: string;
    unbalanced: string;
    newEntry: string;
    postToGl: string;
    posted: string;
    unposted: string;
    entriesCount: string;
    allStatusesAll: string;
    onlyPosted: string;
    onlyDraft: string;
    allTransTypes: string;
    postedStatus: string;
    draftStatus: string;
    postSuccess: string;
    postConfirm: string;
    postToGlButton: string;
    registeredTrans: string;
    postedToGlTrans: string;
    draftTrans: string;
    totalTransVolume: string;
    entryRows: string;
    balancedEntry: string;
    unbalancedEntry: string;
    draftNotice: string;
    postedNotice: string;
    lineSeq: string;
    fiscalType: string;
    documentRef: string;
    relatedInvoice: string;
    relatedPayment: string;
    inspect: string;
    entriesLoading: string;
    pageOf: string;
    totalEntriesCount: string;
  };
  reports: {
    title: string;
    subtitle: string;
    balanceSheet: string;
    incomeStatement: string;
    trialBalance: string;
    aging: string;
    assets: string;
    currentAssets: string;
    nonCurrentAssets: string;
    liabilities: string;
    currentLiabilities: string;
    equity: string;
    totalAssets: string;
    totalLiabilities: string;
    totalEquity: string;
    revenue: string;
    expenses: string;
    netIncome: string;
    asOfDate: string;
    generateReport: string;
    allTimesLive: string;
    fiscalYear: string;
    customerReceivablesAR: string;
    vendorPayablesAP: string;
    printButton: string;
    downloadCsvButton: string;
    calculatingReport: string;
    totalDebitAllAccounts: string;
    totalCreditAllAccounts: string;
    differenceAmount: string;
    balanceSheetTitle: string;
    companyLabel: string;
    assetsI: string;
    currentAssetsA: string;
    longTermAssetsB: string;
    totalAssetsLabel: string;
    liabilitiesII: string;
    currentLiabilitiesA: string;
    longTermLiabilitiesB: string;
    equityC: string;
    totalLiabilitiesAndEquity: string;
    cogs: string;
    grossProfit: string;
    netProfitPeriod: string;
    detailedIncomeStatement: string;
    revenuesSection: string;
    cogsSection: string;
    grossOperatingProfit: string;
    operatingExpensesOpex: string;
    netProfitOrLoss: string;
    notDueYet: string;
    days1to30: string;
    days31to60: string;
    days61to90: string;
    days90Plus: string;
    openReceivables: string;
    openPayables: string;
    noAgingParties: string;
    partyTitle: string;
    csvReadyNotice: string;
    searchAccountPlaceholder: string;
    cashFlow: string;
    comparativeBalanceSheet: string;
    comparativeIncomeStatement: string;
    operatingActivities: string;
    investingActivities: string;
    financingActivities: string;
    netCashChange: string;
    openingCash: string;
    closingCash: string;
    period1: string;
    period2: string;
    difference: string;
    percentChange: string;
    inventoryValuation: string;
    pastDue: string;
  };
  financialAccounts: {
    title: string;
    subtitle: string;
    tabs: {
      accounts: string;
      transactions: string;
      reconciliation: string;
    };
    stats: {
      totalAccounts: string;
      totalAccountsSub: string;
      activeBalance: string;
      activeBalanceSub: string;
      creditLimit: string;
      creditLimitSub: string;
      pendingRec: string;
      pendingRecSub: string;
      totalTrans: string;
      totalTransSub: string;
      totalInflow: string;
      totalInflowSub: string;
      totalOutflow: string;
      totalOutflowSub: string;
      netChange: string;
      netChangeSub: string;
      totalRecs: string;
      openRecs: string;
      matchedTrans: string;
      recDiff: string;
    };
    accounts: {
      newAccount: string;
      editAccount: string;
      accountDetail: string;
      accountId: string;
      accountName: string;
      accountType: string;
      currency: string;
      balance: string;
      availableBalance: string;
      actualBalance: string;
      searchPlaceholder: string;
      allTypes: string;
      allStatuses: string;
      noAccounts: string;
      ownerParty: string;
      glAccount: string;
      isPositive: string;
      active: string;
      inactive: string;
      replenishLevel: string;
    };
    transactions: {
      newTrans: string;
      transfer: string;
      transId: string;
      transType: string;
      transDate: string;
      entryDate: string;
      amount: string;
      party: string;
      reason: string;
      searchPlaceholder: string;
      noTrans: string;
      deposit: string;
      withdrawal: string;
      adjustment: string;
      fee: string;
      interest: string;
      allAccounts: string;
      allTypes: string;
      allStatuses: string;
      fromAccount: string;
      toAccount: string;
      transferAmount: string;
    };
    reconciliation: {
      newRec: string;
      recId: string;
      recName: string;
      glAccountId: string;
      openingBalance: string;
      closingBalance: string;
      reconciledBalance: string;
      difference: string;
      status: string;
      noRecs: string;
      manageTrans: string;
      linkTrans: string;
      unlinkTrans: string;
      linkedTrans: string;
      unlinkedTrans: string;
      reconcileNow: string;
      reconciledSuccess: string;
    };
    messages: {
      loadAccountsError: string;
      loadTransError: string;
      loadRecError: string;
      accountSaved: string;
      accountUpdated: string;
      transSaved: string;
      transferSuccess: string;
      recSaved: string;
      confirmDelete: string;
    };
    rolesAndAuth: {
      rolesTab: string;
      authTab: string;
      addRole: string;
      addAuth: string;
      partyId: string;
      roleType: string;
      fromDate: string;
      thruDate: string;
      amount: string;
      authDate: string;
      active: string;
      expired: string;
      expireBtn: string;
      noRoles: string;
      noAuths: string;
      deleteRoleConfirm: string;
      expireAuthConfirm: string;
    };
  };
  advancedAccounting: {
    title: string;
    subtitle: string;
    tabs: {
      billingAccounts: string;
      fixedAssets: string;
      budgets: string;
      agreements: string;
    };
    billingAccounts: {
      newAccount: string;
      editAccount: string;
      detail: string;
      accountId: string;
      limit: string;
      billed: string;
      available: string;
      customer: string;
      currency: string;
      fromDate: string;
      thruDate: string;
      searchPlaceholder: string;
      allParties: string;
      noAccounts: string;
      netLimit: string;
      totalLimit: string;
      totalBilled: string;
      totalAvailable: string;
      accountCount: string;
      invoicesCharged: string;
      paymentsApplied: string;
    };
    fixedAssets: {
      newAsset: string;
      editAsset: string;
      detail: string;
      assetId: string;
      name: string;
      type: string;
      cost: string;
      depreciation: string;
      nbv: string;
      purchaseDate: string;
      serialNumber: string;
      searchPlaceholder: string;
      allTypes: string;
      noAssets: string;
      totalAssets: string;
      totalCost: string;
      accumDepreciation: string;
      totalNbv: string;
      calculateDepreciation: string;
      depreciationSuccess: string;
      usefulLife: string;
      salvageValue: string;
      depreciationMethod: string;
    };
    budgets: {
      newBudget: string;
      detail: string;
      addItem: string;
      budgetId: string;
      budgetType: string;
      status: string;
      amount: string;
      period: string;
      searchPlaceholder: string;
      allTypes: string;
      allStatuses: string;
      noBudgets: string;
      totalBudgets: string;
      totalAmount: string;
      draftBudgets: string;
      approvedBudgets: string;
      itemsCount: string;
      updateStatus: string;
      purpose: string;
    };
    agreements: {
      newAgreement: string;
      detail: string;
      agreementId: string;
      type: string;
      partyFrom: string;
      partyTo: string;
      agreementDate: string;
      fromDate: string;
      thruDate: string;
      searchPlaceholder: string;
      allTypes: string;
      noAgreements: string;
      totalAgreements: string;
      salesAgreements: string;
      purchaseAgreements: string;
      activeAgreements: string;
      textData: string;
      items: string;
    };
    messages: {
      loadBaError: string;
      loadFaError: string;
      loadBgError: string;
      loadAgError: string;
      baSaved: string;
      baUpdated: string;
      faSaved: string;
      faUpdated: string;
      bgSaved: string;
      bgItemAdded: string;
      bgStatusUpdated: string;
      agSaved: string;
    };
  };
  taxAndGlMapping: {
    title: string;
    subtitle: string;
    tabs: {
      rates: string;
      authorities: string;
      invoiceMappings: string;
      paymentDefaults: string;
    };
    rates: {
      newRate: string;
      rateId: string;
      authority: string;
      rateType: string;
      percentage: string;
      taxCategory: string;
      fromDate: string;
      thruDate: string;
      searchPlaceholder: string;
      noRates: string;
      totalRates: string;
      avgRate: string;
      activeRates: string;
    };
    authorities: {
      newAuthority: string;
      authorityId: string;
      partyId: string;
      geoId: string;
      name: string;
      searchPlaceholder: string;
      noAuthorities: string;
      totalAuthorities: string;
      authorityDetail: string;
      addGlAccount: string;
      glAccountId: string;
      noGlAccounts: string;
    };
    invoiceMappings: {
      newMapping: string;
      itemType: string;
      glAccount: string;
      mappingDesc: string;
      searchPlaceholder: string;
      noMappings: string;
      totalMappings: string;
    };
    paymentDefaults: {
      newDefault: string;
      paymentMethodType: string;
      glAccount: string;
      searchPlaceholder: string;
      noDefaults: string;
      totalDefaults: string;
    };
    messages: {
      loadRatesError: string;
      loadAuthoritiesError: string;
      loadMappingsError: string;
      rateSaved: string;
      authoritySaved: string;
      mappingSaved: string;
      defaultSaved: string;
      deleteSuccess: string;
      confirmDelete: string;
    };
  };
  invoiceNotesAndTerms: {
    notesTab: string;
    termsTab: string;
    notesTitle: string;
    termsTitle: string;
    addNote: string;
    closeForm: string;
    noteTitle: string;
    noteContent: string;
    saveNote: string;
    saving: string;
    noNotes: string;
    deleteNoteConfirm: string;
    addTerm: string;
    termType: string;
    termValue: string;
    termDays: string;
    textValue: string;
    description: string;
    saveTerm: string;
    noTerms: string;
    deleteTermConfirm: string;
    daysSuffix: string;
    notePlaceholder: string;
    contentPlaceholder: string;
    textValuePlaceholder: string;
    descPlaceholder: string;
    noteRequired: string;
    termError: string;
  };
  testPage: {
    title: string;
    description: string;
    sendRequest: string;
    loading: string;
    errorTitle: string;
    errorNote: string;
    responseTitle: string;
  };
  fiscalPeriods: {
    title: string;
    subtitle: string;
    newPeriod: string;
    editPeriod: string;
    closePeriod: string;
    reopenPeriod: string;
    deletePeriod: string;
    periodId: string;
    periodName: string;
    periodType: string;
    parentPeriod: string;
    fromDate: string;
    thruDate: string;
    periodNum: string;
    organization: string;
    status: string;
    open: string;
    closed: string;
    cascadeCloseNotice: string;
    confirmClose: string;
    confirmReopen: string;
    confirmDelete: string;
    stats: {
      totalPeriods: string;
      openPeriods: string;
      closedPeriods: string;
      fiscalYears: string;
    };
    filterAllTypes: string;
    filterAllStatuses: string;
    searchPlaceholder: string;
    noPeriods: string;
    successCreated: string;
    successUpdated: string;
    successClosed: string;
    successReopened: string;
  };
  fxRates: {
    title: string;
    subtitle: string;
    newRate: string;
    sourceCurrency: string;
    targetCurrency: string;
    rate: string;
    effectiveDate: string;
    thruDate: string;
    purpose: string;
    status: string;
    active: string;
    expired: string;
    calculator: string;
    convertAmount: string;
    result: string;
    stats: {
      totalConversions: string;
      activeRates: string;
      currenciesCount: string;
      baseCurrency: string;
    };
    searchPlaceholder: string;
    noRates: string;
    confirmDelete: string;
    successCreated: string;
    successDeleted: string;
  };
  costCenters: {
    title: string;
    subtitle: string;
    newCategory: string;
    addAccount: string;
    removeAccount: string;
    categoryName: string;
    categoryType: string;
    memberAccounts: string;
    allocPercent: string;
    debit: string;
    credit: string;
    balance: string;
    stats: {
      totalCenters: string;
      assignedAccounts: string;
      totalAllocDebit: string;
      netExpense: string;
    };
    noCostCenters: string;
    confirmRemove: string;
    successCreated: string;
    successAccountAdded: string;
    successAccountRemoved: string;
  };
  depositSlips: {
    title: string;
    subtitle: string;
    newSlip: string;
    cancelSlip: string;
    confirmCancel: string;
    slipId: string;
    slipName: string;
    bankAccount: string;
    transStatus: string;
    paymentCount: string;
    totalAmount: string;
    createdDate: string;
    undepositedPayments: string;
    selectPaymentsToDeposit: string;
    totalSelected: string;
    depositTo: string;
    viewDetail: string;
    memberPayments: string;
    noDepositSlips: string;
    noUndepositedPayments: string;
    stats: {
      totalSlips: string;
      totalDepositedAmount: string;
      undepositedCount: string;
      undepositedAmount: string;
    };
    messages: {
      successCreated: string;
      successCancelled: string;
      selectAtLeastOne: string;
    };
  };
  accountingPreferences: {
    title: string;
    subtitle: string;
    orgSelector: string;
    saveChanges: string;
    tabs: {
      general: string;
      sequencing: string;
      journals: string;
    };
    stats: {
      baseCurrency: string;
      fiscalYearStart: string;
      totalJournals: string;
      invoiceSequence: string;
    };
    general: {
      sectionTitle: string;
      sectionSubtitle: string;
      baseCurrency: string;
      baseCurrencyHint: string;
      fiscalStartMonth: string;
      fiscalStartDay: string;
      taxForm: string;
      cogsMethod: string;
      errorJournal: string;
      errorJournalHint: string;
      enableAccounting: string;
      enableAccountingHint: string;
    };
    sequencing: {
      sectionTitle: string;
      sectionSubtitle: string;
      invoicePrefix: string;
      lastInvoiceNo: string;
      invoiceSeqMethod: string;
      useInvoiceIdForReturns: string;
      quotePrefix: string;
      lastQuoteNo: string;
      quoteSeqMethod: string;
      orderPrefix: string;
      lastOrderNo: string;
      orderSeqMethod: string;
    };
    journals: {
      sectionTitle: string;
      sectionSubtitle: string;
      newJournal: string;
      editJournal: string;
      deleteJournal: string;
      postJournal: string;
      journalId: string;
      journalName: string;
      isPosted: string;
      postedDate: string;
      transCount: string;
      debitTotal: string;
      creditTotal: string;
      balanceDiff: string;
      noJournals: string;
      confirmDelete: string;
      confirmPost: string;
      draft: string;
      posted: string;
    };
    messages: {
      successSaved: string;
      successCreatedJournal: string;
      successUpdatedJournal: string;
      successDeletedJournal: string;
      successPostedJournal: string;
    };
  };
  paymentGateways: {
    title: string;
    subtitle: string;
    tabs: {
      responses: string;
      configs: string;
    };
    stats: {
      totalTrans: string;
      capturedVolume: string;
      approvedCount: string;
      declinedCount: string;
    };
    filters: {
      allServiceTypes: string;
      allMethodTypes: string;
      allStatuses: string;
      approved: string;
      captured: string;
      declined: string;
      searchPlaceholder: string;
    };
    responses: {
      responseId: string;
      serviceType: string;
      methodType: string;
      referenceNum: string;
      gatewayCode: string;
      gatewayFlag: string;
      amount: string;
      date: string;
      status: string;
      message: string;
      noResponses: string;
      viewDetail: string;
    };
    detail: {
      title: string;
      responseId: string;
      serviceType: string;
      orderPrefId: string;
      referenceNum: string;
      altReference: string;
      subReference: string;
      gatewayCode: string;
      gatewayFlag: string;
      gatewayMessage: string;
      avsResult: string;
      cvvResult: string;
      scoreResult: string;
      date: string;
      declined: string;
      nsf: string;
      badExpire: string;
      badCardNumber: string;
      rawMessages: string;
      noRawMessages: string;
      linkedPayments: string;
      noLinkedPayments: string;
    };
    configs: {
      configId: string;
      configType: string;
      description: string;
      newConfig: string;
      editConfig: string;
      deleteConfig: string;
      noConfigs: string;
      confirmDelete: string;
    };
    messages: {
      successSavedConfig: string;
      successDeletedConfig: string;
    };
  };
  checkRun: {
    title: string;
    subtitle: string;
    tabs: {
      checkRuns: string;
      newCheckRun: string;
    };
    stats: {
      totalRuns: string;
      issuedChecks: string;
      voidedChecks: string;
      totalVolume: string;
    };
    table: {
      groupId: string;
      name: string;
      date: string;
      checkCount: string;
      amount: string;
      status: string;
      actions: string;
      noRuns: string;
    };
    detail: {
      title: string;
      groupInfo: string;
      cancelRun: string;
      confirmCancelRun: string;
      printChecks: string;
      checksList: string;
      checkNumber: string;
      payee: string;
      amount: string;
      status: string;
      appliedInvoices: string;
      voidCheck: string;
      confirmVoidCheck: string;
      noChecks: string;
    };
    wizard: {
      title: string;
      bankAccount: string;
      selectBankAccount: string;
      startCheckNumber: string;
      startCheckNumberHelp: string;
      paymentDate: string;
      runName: string;
      runNamePlaceholder: string;
      filterVendor: string;
      allVendors: string;
      dueDateBefore: string;
      availableInvoices: string;
      noPayableInvoices: string;
      selectedCount: string;
      selectedTotal: string;
      selectAll: string;
      deselectAll: string;
      invoiceId: string;
      vendor: string;
      invoiceDate: string;
      dueDate: string;
      openAmount: string;
      createRunButton: string;
    };
    voucher: {
      title: string;
      payer: string;
      bank: string;
      accountNo: string;
      payToOrderOf: string;
      date: string;
      checkNo: string;
      dollars: string;
      amountWords: string;
      memo: string;
      authorizedSignature: string;
      voucherStub: string;
      invoiceRef: string;
      paidAmount: string;
      print: string;
      close: string;
    };
    messages: {
      createdSuccess: string;
      cancelledSuccess: string;
      voidedSuccess: string;
      selectAtLeastOne: string;
      selectBank: string;
    };
  };
  commissionRun: {
    title: string;
    subtitle: string;
    tabs: {
      invoices: string;
      newRun: string;
      inventoryValuation: string;
      pastDue: string;
    };
    stats: {
      totalVolume: string;
      invoicesGenerated: string;
      activeReps: string;
      pendingSales: string;
    };
    table: {
      invoiceId: string;
      salesRep: string;
      date: string;
      amount: string;
      status: string;
      sources: string;
      actions: string;
      noInvoices: string;
    };
    wizard: {
      title: string;
      salesRep: string;
      allReps: string;
      commissionRate: string;
      description: string;
      descriptionPlaceholder: string;
      fromDate: string;
      thruDate: string;
      availableInvoices: string;
      noEligibleInvoices: string;
      selectedCount: string;
      selectedCommission: string;
      selectAll: string;
      deselectAll: string;
      invoiceId: string;
      customer: string;
      salesAmount: string;
      rate: string;
      commission: string;
      executeButton: string;
    };
    inventory: {
      title: string;
      facility: string;
      allFacilities: string;
      totalSkus: string;
      totalQty: string;
      totalValuation: string;
      productId: string;
      productName: string;
      warehouse: string;
      quantity: string;
      unitCost: string;
      valuation: string;
      noInventory: string;
    };
    pastDue: {
      title: string;
      invoiceType: string;
      allTypes: string;
      totalPastDue: string;
      totalDueSoon: string;
      aging1_30: string;
      aging31_60: string;
      aging61_90: string;
      aging90_plus: string;
      partner: string;
      dueDate: string;
      daysOverdue: string;
      openAmount: string;
      noPastDue: string;
    };
    messages: {
      createdSuccess: string;
      selectAtLeastOne: string;
    };
  };
  fixedAssetLifecycle: {
    title: string;
    subtitle: string;
    tabs: {
      overview: string;
      depreciation: string;
      maintenance: string;
      meters: string;
      assignments: string;
      registrations: string;
    };
    depreciation: {
      title: string;
      method: string;
      usefulYears: string;
      purchaseCost: string;
      salvageValue: string;
      accumulatedDep: string;
      netBookValue: string;
      nextSuggestedAmount: string;
      postToGl: string;
      postModalTitle: string;
      amountToPost: string;
      debitGlAccount: string;
      creditGlAccount: string;
      description: string;
      postingSuccess: string;
      scheduleTitle: string;
      year: string;
      depAmount: string;
      accumAmount: string;
      endingNbv: string;
      historyTitle: string;
      transId: string;
      date: string;
      status: string;
      posted: string;
      unposted: string;
      noHistory: string;
      batchTitle: string;
      batchDescription: string;
      runBatchBtn: string;
      batchConfirm: string;
      batchSuccess: string;
    };
    maintenance: {
      title: string;
      newTask: string;
      maintSeq: string;
      maintType: string;
      date: string;
      interval: string;
      comments: string;
      poNumber: string;
      status: string;
      noMaints: string;
      changeStatus: string;
      created: string;
      inProcess: string;
      completed: string;
      cancelled: string;
    };
    meters: {
      title: string;
      newReading: string;
      meterType: string;
      readingDate: string;
      meterValue: string;
      odometer: string;
      hours: string;
      noMeters: string;
    };
    assignments: {
      title: string;
      newAssignment: string;
      party: string;
      partyName: string;
      role: string;
      fromDate: string;
      thruDate: string;
      allocatedCost: string;
      comments: string;
      status: string;
      active: string;
      released: string;
      releaseBtn: string;
      releaseConfirm: string;
      noAssignments: string;
    };
    registrations: {
      title: string;
      newRegistration: string;
      regNumber: string;
      licensePlate: string;
      govAgency: string;
      regDate: string;
      identType: string;
      identValue: string;
      vin: string;
      serialNumber: string;
      noRegistrations: string;
      noIdents: string;
    };
  };
  budgetAndAgreement: {
    budget: {
      varianceTitle: string;
      varianceSubtitle: string;
      totalBudget: string;
      totalActual: string;
      totalVariance: string;
      overallUsage: string;
      onTrack: string;
      warning: string;
      overBudget: string;
      revisionsTitle: string;
      newRevision: string;
      revisionSeq: string;
      dateRevised: string;
      revisionReason: string;
      revisedAmount: string;
      noRevisions: string;
      workflowTitle: string;
      approve: string;
      review: string;
      reject: string;
      draft: string;
      approved: string;
      reviewed: string;
      rejected: string;
      deleteItemConfirm: string;
      editItem: string;
      varianceReportBtn: string;
      revisionsBtn: string;
    };
    agreement: {
      extendedTitle: string;
      tabs: {
        overview: string;
        items: string;
        terms: string;
        productPrices: string;
        parties: string;
        statusHistory: string;
      };
      newProductPrice: string;
      product: string;
      contractPrice: string;
      noProductPrices: string;
      newItem: string;
      newTerm: string;
      termType: string;
      termDays: string;
      termValue: string;
      changeStatus: string;
      active: string;
      cancelled: string;
      terminated: string;
      inProcess: string;
      noItems: string;
      noTerms: string;
      noParties: string;
      statusDate: string;
      statusUser: string;
      comments: string;
    };
  };
  billingAccountExtended: {
    title: string;
    subtitle: string;
    tabs: {
      overview: string;
      statement: string;
      roles: string;
      terms: string;
      payments: string;
    };
    statement: {
      dateRange: string;
      allDates: string;
      filter: string;
      totalDebits: string;
      totalCredits: string;
      netBalance: string;
      availableCredit: string;
      utilization: string;
      runningBalance: string;
      entryDate: string;
      entryType: string;
      refNum: string;
      description: string;
      debit: string;
      credit: string;
      balance: string;
      noEntries: string;
      printStatement: string;
      invoice: string;
      payment: string;
    };
    roles: {
      title: string;
      newRole: string;
      party: string;
      roleType: string;
      fromDate: string;
      thruDate: string;
      noRoles: string;
      deleteRoleConfirm: string;
    };
    terms: {
      title: string;
      newTerm: string;
      termType: string;
      termDays: string;
      termValue: string;
      description: string;
      noTerms: string;
      deleteTermConfirm: string;
    };
    payments: {
      title: string;
      applyPayment: string;
      paymentId: string;
      amountApplied: string;
      effectiveDate: string;
      partyFrom: string;
      noPayments: string;
      removeApplicationConfirm: string;
    };
  };
  invoiceBatch: {
    selectedCount: string;
    clearSelection: string;
    approveSelected: string;
    readySelected: string;
    cancelSelected: string;
    copyInvoice: string;
    copySuccess: string;
    batchSuccess: string;
  };
  taxSettlement: {
    modalTitle: string;
    modalSubtitle: string;
    tabs: {
      report: string;
      parties: string;
      categories: string;
      glAccounts: string;
    };
    report: {
      dateRange: string;
      allDates: string;
      filter: string;
      totalSalesBase: string;
      outputTax: string;
      totalPurchasesBase: string;
      inputTax: string;
      netTaxDue: string;
      payable: string;
      carriedForward: string;
      ratesBreakdown: string;
      rateName: string;
      salesTax: string;
      purchaseTax: string;
      netAmount: string;
      invoicesList: string;
      invoiceId: string;
      invoiceDate: string;
      party: string;
      taxableBase: string;
      taxAmount: string;
      type: string;
      printReport: string;
      noItems: string;
    };
    parties: {
      title: string;
      newParty: string;
      partyId: string;
      taxId: string;
      isExempt: string;
      isNexus: string;
      fromDate: string;
      thruDate: string;
      noParties: string;
      deleteConfirm: string;
    };
    categories: {
      title: string;
      newCategory: string;
      categoryId: string;
      categoryName: string;
      description: string;
      noCategories: string;
      deleteConfirm: string;
    };
  };
  partyStatement: {
    title: string;
    subtitle: string;
    tabs: {
      summary: string;
      ledger: string;
      openInvoices: string;
      unappliedPayments: string;
    };
    summary: {
      totalInvoiced: string;
      totalPaid: string;
      openBalance: string;
      agingTitle: string;
      current: string;
      days1_30: string;
      days31_60: string;
      days61_90: string;
      days90Plus: string;
      totalOverdue: string;
    };
    ledger: {
      dateRange: string;
      allDates: string;
      filter: string;
      date: string;
      type: string;
      refNum: string;
      description: string;
      debit: string;
      credit: string;
      balance: string;
      printLedger: string;
      noTransactions: string;
      invoice: string;
      payment: string;
    };
    openInvoices: {
      title: string;
      invoiceId: string;
      date: string;
      dueDate: string;
      total: string;
      outstanding: string;
      daysOverdue: string;
      noOpenInvoices: string;
    };
    unappliedPayments: {
      title: string;
      paymentId: string;
      date: string;
      amount: string;
      unapplied: string;
      noUnapplied: string;
    };
  };
  extendedGl: {
    title: string;
    subtitle: string;
    tabs: {
      varianceReasons: string;
      partyGl: string;
      creditCardGl: string;
      fixedAssetGl: string;
      finAccountGl: string;
      categoryGl: string;
    };
    variance: {
      newMapping: string;
      reason: string;
      glAccount: string;
      noMappings: string;
      deleteConfirm: string;
    };
    party: {
      newMapping: string;
      party: string;
      partyId: string;
      role: string;
      accountType: string;
      glAccount: string;
      noMappings: string;
      deleteConfirm: string;
    };
    creditCard: {
      newMapping: string;
      cardType: string;
      glAccount: string;
      noMappings: string;
      deleteConfirm: string;
    };
    fixedAsset: {
      newMapping: string;
      assetType: string;
      assetGl: string;
      accDepGl: string;
      depGl: string;
      profitGl: string;
      lossGl: string;
      noMappings: string;
      deleteConfirm: string;
    };
    finAccountType: {
      newMapping: string;
      finAccountType: string;
      glAccount: string;
      noMappings: string;
      deleteConfirm: string;
    };
    categoryGl: {
      newMapping: string;
      category: string;
      glAccountType: string;
      glAccount: string;
      noMappings: string;
      deleteConfirm: string;
    };
  };
  batchJournal: {
    selectedCount: string;
    postSelected: string;
    posting: string;
    confirmTitle: string;
    confirmDesc: string;
    selectAllDraft: string;
    clearSelection: string;
    onlyDraftsCanBePosted: string;
  };
  costCalcs: {
    title: string;
    subtitle: string;
    newCalc: string;
    editCalc: string;
    calcId: string;
    description: string;
    costGlType: string;
    offsettingGlType: string;
    fixedCost: string;
    variableCost: string;
    perMilliSecond: string;
    currency: string;
    noCalcs: string;
    deleteConfirm: string;
  };
  modulePlaceholder: {
    statusBadge: string;
    backToAccounting: string;
    featuresTitle: string;
    ofbizIntegration: string;
    ofbizIntegrationDesc: string;
    ordersDescription: string;
    ordersFeatures: string[];
    manufacturingDescription: string;
    manufacturingFeatures: string[];
    inventoryDescription: string;
    inventoryFeatures: string[];
  };
  parties: {
    title: string;
    subtitle: string;
    createNew: string;
    totalParties: string;
    activeCustomers: string;
    activeSuppliers: string;
    corporate: string;
    individual: string;
    searchPlaceholder: string;
    tabAll: string;
    tabCustomers: string;
    tabSuppliers: string;
    tabEmployees: string;
    tabDisabled: string;
    partyId: string;
    name: string;
    type: string;
    roles: string;
    contact: string;
    city: string;
    status: string;
    actions: string;
    corporateType: string;
    individualType: string;
    viewProfile: string;
    viewStatement: string;
    editParty: string;
    activate: string;
    deactivate: string;
    activeStatus: string;
    disabledStatus: string;
    filterType: string;
    filterRole: string;
    filterStatus: string;
    allTypes: string;
    allRoles: string;
    allStatuses: string;
    noPartiesFound: string;
    partyDetails: string;
    basicInfo: string;
    createdDate: string;
    taxAndIdNumbers: string;
    rolesAndClass: string;
    addRole: string;
    removeRole: string;
    selectRole: string;
    roleAddedSuccess: string;
    roleRemovedSuccess: string;
    confirmDeleteRole: string;
    noAvailableRoles: string;
    contactMechs: string;
    postalAddresses: string;
    telecomNumbers: string;
    emailAddresses: string;
    connectedPersons: string;
    financialSummary: string;
    totalInvoices: string;
    totalPayments: string;
    addAddress: string;
    addPhone: string;
    addEmail: string;
    addIdentification: string;
    addRelationship: string;
    noAddresses: string;
    noPhones: string;
    noEmails: string;
    noIdentifications: string;
    noRelationships: string;
    idType: string;
    idNumber: string;
    fullAddress: string;
    purpose: string;
    relationshipType: string;
    connectedParty: string;
    notes: string;
    createModalTitle: string;
    corporateTab: string;
    individualTab: string;
    groupName: string;
    groupNamePlaceholder: string;
    groupNameLocal: string;
    officeSiteName: string;
    firstName: string;
    lastName: string;
    titleField: string;
    gender: string;
    male: string;
    female: string;
    birthDate: string;
    initialRole: string;
    vknTckn: string;
    phone: string;
    email: string;
    address: string;
    postalCode: string;
    country: string;
    customPartyIdOptional: string;
    saveSuccess: string;
    updateSuccess: string;
    statusSuccess: string;
    // Phase 2 keys
    tabOverview: string;
    tabFinancial: string;
    tabTerms: string;
    tabSegments: string;
    tabNotes: string;
    creditLimit: string;
    accountBalance: string;
    availableBalance: string;
    receivables: string;
    payables: string;
    netExposure: string;
    riskSafe: string;
    riskWarning: string;
    riskExceeded: string;
    riskNoLimit: string;
    riskStatus: string;
    editCreditLimit: string;
    billingAccounts: string;
    currency: string;
    limitUsage: string;
    paymentTermsTitle: string;
    addPaymentTerm: string;
    termType: string;
    termDays: string;
    termValue: string;
    noPaymentTerms: string;
    segmentsTitle: string;
    addSegment: string;
    selectSegment: string;
    noSegments: string;
    crmNotesTitle: string;
    addNote: string;
    noteTitle: string;
    noteContent: string;
    author: string;
    date: string;
    noNotes: string;
    taxAuthInfo: string;
    exemptFromTax: string;
    taxPayerNumber: string;
    saveTaxAuth: string;
    // Phase 3 & Final keys
    tabPayments: string;
    tabDocuments: string;
    tabAttributes: string;
    tabUserLogins: string;
    bankAccounts: string;
    creditCards: string;
    addBankAccount: string;
    addCreditCard: string;
    bankName: string;
    iban: string;
    accountHolder: string;
    routingNumber: string;
    cardNumber: string;
    expireDate: string;
    cardType: string;
    nameOnCard: string;
    noBankAccounts: string;
    noCreditCards: string;
    attributesTitle: string;
    addAttribute: string;
    attrName: string;
    attrValue: string;
    attrDescription: string;
    noAttributes: string;
    documentsTitle: string;
    addDocument: string;
    documentType: string;
    documentName: string;
    noDocuments: string;
    userLoginsTitle: string;
    addUserLogin: string;
    username: string;
    password: string;
    securityGroup: string;
    userStatus: string;
    noUserLogins: string;
    exportCsv: string;
    exportSuccess: string;
  };
  auth: {
    loginTitle: string;
    loginSubtitle: string;
    username: string;
    usernamePlaceholder: string;
    password: string;
    passwordPlaceholder: string;
    loginButton: string;
    loggingIn: string;
    logoutConfirmTitle: string;
    logoutConfirmMessage: string;
    logoutSuccess: string;
    invalidCredentials: string;
    emptyFieldsError: string;
    demoCredentialsTitle: string;
    demoCredentialsHint: string;
    changePassword: string;
    currentPassword: string;
    newPassword: string;
    confirmPassword: string;
    savePassword: string;
    savingPassword: string;
    passwordChangedSuccess: string;
    passwordMismatch: string;
    userProfile: string;
    adminBadge: string;
    userBadge: string;
    securityGroups: string;
    sessionActive: string;
    tenantId: string;
    tenantIdPlaceholder: string;
    tenantOptional: string;
    activeTenantBadge: string;
  };
  users: {
    title: string;
    subtitle: string;
    tabUsers: string;
    tabSecurityGroups: string;
    createUser: string;
    createGroup: string;
    searchPlaceholder: string;
    filterAll: string;
    filterActive: string;
    filterDisabled: string;
    filterLocked: string;
    userLoginId: string;
    displayName: string;
    status: string;
    securityGroups: string;
    failedLogins: string;
    locked: string;
    unlocked: string;
    unlockAccount: string;
    deactivateAccount: string;
    activateAccount: string;
    resetPassword: string;
    addSecurityGroup: string;
    removeSecurityGroup: string;
    noUsersFound: string;
    userDetail: string;
    accountSecurity: string;
    lastLogin: string;
    permissionsTitle: string;
    groupPermissions: string;
    assignedPermissions: string;
    availablePermissions: string;
    addPermission: string;
    removePermission: string;
    groupCode: string;
    groupDescription: string;
    permissionCount: string;
    userCount: string;
    requirePasswordChange: string;
    partyLink: string;
    selectParty: string;
    selectGroup: string;
    password: string;
    passwordVerify: string;
    createdSuccess: string;
    updatedSuccess: string;
    resetPasswordSuccess: string;
    groupCreatedSuccess: string;
    tabRoleTypes: string;
    createRoleType: string;
    roleTypeCode: string;
    roleTypeDescription: string;
    parentRoleType: string;
    partyCount: string;
    hasTable: string;
    searchRoleTypesPlaceholder: string;
    roleTypeCreatedSuccess: string;
    roleTypeDeletedSuccess: string;
    confirmDeleteRoleType: string;
    noRoleTypesFound: string;
    noParentRole: string;
    tabActiveSessions: string;
    tabLoginHistory: string;
    totalActiveSessions: string;
    uniqueActiveUsers: string;
    clientIp: string;
    userAgent: string;
    sessionId: string;
    sessionStart: string;
    lastRequest: string;
    webapp: string;
    onlineNow: string;
    searchSessionsPlaceholder: string;
    noActiveSessionsFound: string;
    temporaryAssignment: string;
    thruDateOptional: string;
    indefinite: string;
    expiresOn: string;
    expired: string;
    loginSuccessful: string;
    loginFailed: string;
    noLoginHistoryFound: string;
    refreshSessions: string;
  };

  systemAdmin: {
    title: string;
    subtitle: string;
    tabCache: string;
    tabJobs: string;
    tabDiagnostics: string;
    totalCaches: string;
    totalCacheMemory: string;
    jvmMemoryUsage: string;
    totalHits: string;
    totalMisses: string;
    hitRatio: string;
    clearAllCaches: string;
    clearAllConfirm: string;
    forceGc: string;
    forceGcSuccess: string;
    cacheName: string;
    cacheSize: string;
    hitCount: string;
    missCount: string;
    expireTime: string;
    maxInMemory: string;
    memoryBytes: string;
    clearCache: string;
    clearCacheSuccess: string;
    searchCachePlaceholder: string;
    noCachesFound: string;
    totalJobs: string;
    pendingJobs: string;
    runningJobs: string;
    finishedJobs: string;
    failedJobs: string;
    triggerService: string;
    triggerServicePrompt: string;
    serviceName: string;
    serviceTriggeredSuccess: string;
    jobId: string;
    jobName: string;
    runTime: string;
    startDateTime: string;
    finishDateTime: string;
    cancelJob: string;
    resetJob: string;
    jobCancelledSuccess: string;
    jobResetSuccess: string;
    searchJobsPlaceholder: string;
    filterAll: string;
    filterPending: string;
    filterRunning: string;
    filterFinished: string;
    filterFailed: string;
    filterCancelled: string;
    noJobsFound: string;
    retryCount: string;
    jvmTitle: string;
    vmName: string;
    vmVendor: string;
    vmVersion: string;
    uptime: string;
    startTime: string;
    osTitle: string;
    osName: string;
    osArch: string;
    processors: string;
    systemLoad: string;
    memoryTitle: string;
    heapMemory: string;
    nonHeapMemory: string;
    threadsTitle: string;
    liveThreads: string;
    peakThreads: string;
    daemonThreads: string;
    ofbizTitle: string;
    delegator: string;
    serverTime: string;
    tabTenants: string;
    totalTenants: string;
    activeTenants: string;
    disabledTenants: string;
    createTenant: string;
    editTenant: string;
    deleteTenant: string;
    tenantId: string;
    tenantName: string;
    initialPath: string;
    domainsCount: string;
    componentsCount: string;
    dataSourcesCount: string;
    searchTenantsPlaceholder: string;
    noTenantsFound: string;
    tenantCreatedSuccess: string;
    tenantUpdatedSuccess: string;
    tenantDeletedSuccess: string;
    confirmDeleteTenant: string;
    domainsTitle: string;
    componentsTitle: string;
    dataSourcesTitle: string;
    addDomain: string;
    domainName: string;
    domainAddedSuccess: string;
    domainDeletedSuccess: string;
    confirmDeleteDomain: string;
    noDomainsFound: string;
    noComponentsFound: string;
    noDataSourcesFound: string;
    tenantDetailTitle: string;
  };
  products: {
    title: string;
    subtitle: string;
    createProduct: string;
    editProduct: string;
    deleteProduct: string;
    productId: string;
    productName: string;
    internalName: string;
    productType: string;
    description: string;
    longDescription: string;
    isVirtual: string;
    isVariant: string;
    taxable: string;
    chargeShipping: string;
    returnable: string;
    primaryCategory: string;
    uom: string;
    pricing: string;
    prices: string;
    addPrice: string;
    editPrice: string;
    deletePrice: string;
    priceType: string;
    pricePurpose: string;
    price: string;
    taxInPrice: string;
    fromDate: string;
    thruDate: string;
    identifications: string;
    addIdentification: string;
    deleteIdentification: string;
    idType: string;
    idValue: string;
    attributes: string;
    addAttribute: string;
    editAttribute: string;
    deleteAttribute: string;
    attrName: string;
    attrValue: string;
    attrDescription: string;
    inventorySummary: string;
    qoh: string;
    atp: string;
    facility: string;
    noProductsFound: string;
    searchPlaceholder: string;
    totalProducts: string;
    finishedGoods: string;
    services: string;
    inStockProducts: string;
    productCreatedSuccess: string;
    productUpdatedSuccess: string;
    productDeletedSuccess: string;
    priceAddedSuccess: string;
    priceDeletedSuccess: string;
    idAddedSuccess: string;
    idDeletedSuccess: string;
    attrSavedSuccess: string;
    attrDeletedSuccess: string;
    confirmDeleteProduct: string;
    tabs: {
      general: string;
      prices: string;
      identifications: string;
      attributes: string;
      inventory: string;
    };
  };
  catalogCategories: {
    title: string;
    subtitle: string;
    tabCategories: string;
    tabCatalogs: string;
    tabTree: string;
    totalCatalogs: string;
    totalCategories: string;
    rootCategories: string;
    totalAssignedProducts: string;
    createCatalog: string;
    editCatalog: string;
    deleteCatalog: string;
    catalogId: string;
    catalogName: string;
    useQuickAdd: string;
    viewAllowPermReqd: string;
    purchaseAllowPermReqd: string;
    assignedCategories: string;
    addCategoryToCatalog: string;
    removeCategoryFromCatalog: string;
    catalogCategoryType: string;
    sequenceNum: string;
    createCategory: string;
    editCategory: string;
    deleteCategory: string;
    categoryId: string;
    categoryName: string;
    categoryType: string;
    parentCategory: string;
    noParentCategory: string;
    memberCount: string;
    childCategories: string;
    parentCategories: string;
    memberProducts: string;
    addMemberProduct: string;
    removeMemberProduct: string;
    selectProduct: string;
    searchProductsPlaceholder: string;
    searchCategoriesPlaceholder: string;
    searchCatalogsPlaceholder: string;
    noCatalogsFound: string;
    noCategoriesFound: string;
    noMembersFound: string;
    noChildrenFound: string;
    expandAll: string;
    collapseAll: string;
    treeView: string;
    listView: string;
    catalogCreatedSuccess: string;
    catalogUpdatedSuccess: string;
    catalogDeletedSuccess: string;
    categoryCreatedSuccess: string;
    categoryUpdatedSuccess: string;
    categoryDeletedSuccess: string;
    categoryAssignedSuccess: string;
    categoryRemovedSuccess: string;
    memberAssignedSuccess: string;
    memberRemovedSuccess: string;
    confirmDeleteCatalog: string;
    confirmDeleteCategory: string;
    confirmRemoveCategoryFromCatalog: string;
    confirmRemoveMemberProduct: string;
    categoryDetail: string;
    catalogDetail: string;
    primaryProduct: string;
    quantity: string;
  };
  productVariantsAssocs: {
    title: string;
    subtitle: string;
    tabAssocsAndVariants: string;
    tabFeatures: string;
    tabFeatureCategories: string;
    totalFeatures: string;
    totalFeatureCategories: string;
    totalFeatureAppls: string;
    totalAssocs: string;
    quickVariantWizard: string;
    createAssoc: string;
    createFeature: string;
    createFeatureCategory: string;
    applyFeature: string;
    sourceProduct: string;
    targetProduct: string;
    assocType: string;
    featureName: string;
    featureCategory: string;
    featureType: string;
    idCode: string;
    defaultAmount: string;
    defaultSequence: string;
    applType: string;
    virtualProduct: string;
    variantProduct: string;
    variantName: string;
    selectFeatures: string;
    distinguishingFeatures: string;
    outgoingAssocs: string;
    incomingAssocs: string;
    reason: string;
    quantity: string;
    applCount: string;
    noFeaturesFound: string;
    noAssocsFound: string;
    noFeatureCategoriesFound: string;
    featureCreatedSuccess: string;
    featureUpdatedSuccess: string;
    featureDeletedSuccess: string;
    featureCategoryCreatedSuccess: string;
    featureAppliedSuccess: string;
    featureRemovedSuccess: string;
    assocCreatedSuccess: string;
    assocDeletedSuccess: string;
    variantCreatedSuccess: string;
    confirmDeleteFeature: string;
    confirmDeleteAssoc: string;
    confirmRemoveFeature: string;
    searchFeaturesPlaceholder: string;
    searchAssocsPlaceholder: string;
  };
  pricePromoStore: {
    priceRulesTab: string;
    promosTab: string;
    storesTab: string;
    priceRulesTitle: string;
    promosTitle: string;
    storesTitle: string;
    totalPriceRules: string;
    saleRules: string;
    totalPromotions: string;
    totalStores: string;
    ruleId: string;
    ruleName: string;
    isSale: string;
    conditions: string;
    actions: string;
    createPriceRule: string;
    editPriceRule: string;
    priceRuleDetail: string;
    noPriceRulesFound: string;
    addCondition: string;
    addAction: string;
    inputParam: string;
    operator: string;
    condValue: string;
    actionType: string;
    rateOrAmount: string;
    confirmDeletePriceRule: string;
    confirmDeleteCondition: string;
    confirmDeleteAction: string;
    promoId: string;
    promoName: string;
    promoText: string;
    requireCode: string;
    showToCustomer: string;
    limits: string;
    limitPerOrder: string;
    limitPerCustomer: string;
    limitPerPromo: string;
    codeCount: string;
    ruleCount: string;
    createPromo: string;
    editPromo: string;
    promoDetail: string;
    noPromosFound: string;
    couponCodes: string;
    createCouponCode: string;
    code: string;
    confirmDeletePromo: string;
    confirmDeletePromoCode: string;
    storeId: string;
    storeName: string;
    companyName: string;
    defaultCurrency: string;
    warehouse: string;
    assignedCatalogs: string;
    createStore: string;
    editStore: string;
    storeDetail: string;
    assignCatalog: string;
    noStoresFound: string;
    noCatalogsAssigned: string;
    confirmRemoveCatalog: string;
    ruleCreatedSuccess: string;
    ruleUpdatedSuccess: string;
    ruleDeletedSuccess: string;
    condCreatedSuccess: string;
    condDeletedSuccess: string;
    actionCreatedSuccess: string;
    actionDeletedSuccess: string;
    promoCreatedSuccess: string;
    promoUpdatedSuccess: string;
    promoDeletedSuccess: string;
    codeCreatedSuccess: string;
    codeDeletedSuccess: string;
    storeCreatedSuccess: string;
    storeUpdatedSuccess: string;
    catalogAssignedSuccess: string;
    catalogRemovedSuccess: string;
    searchRulesPlaceholder: string;
    searchPromosPlaceholder: string;
    searchStoresPlaceholder: string;
  };
  inventoryMediaConfig: {
    inventoryTab: string;
    transfersTab: string;
    locationsTab: string;
    configItemsTab: string;
    mediaTab: string;
    advancedInventoryTitle: string;
    configItemsTitle: string;
    productMediaTitle: string;
    totalInventoryItems: string;
    totalQoh: string;
    totalAtp: string;
    totalInventoryValue: string;
    totalTransfers: string;
    totalLocations: string;
    totalConfigItems: string;
    inventoryItemId: string;
    facility: string;
    location: string;
    qoh: string;
    atp: string;
    unitCost: string;
    totalVal: string;
    lotOrSerial: string;
    adjustStock: string;
    noInventoryFound: string;
    adjustStockTitle: string;
    currentQoh: string;
    currentAtp: string;
    qohVariance: string;
    atpVariance: string;
    varianceReason: string;
    adjustSuccess: string;
    transferId: string;
    fromFacility: string;
    toFacility: string;
    transferQty: string;
    sendDate: string;
    createTransfer: string;
    transferStatus: string;
    completeTransfer: string;
    cancelTransfer: string;
    noTransfersFound: string;
    transferCreatedSuccess: string;
    transferStatusUpdatedSuccess: string;
    locationId: string;
    area: string;
    aisle: string;
    section: string;
    level: string;
    position: string;
    itemsInLocation: string;
    createLocation: string;
    noLocationsFound: string;
    confirmDeleteLocation: string;
    locationCreatedSuccess: string;
    locationDeletedSuccess: string;
    mediaManagement: string;
    standardImages: string;
    smallImage: string;
    mediumImage: string;
    largeImage: string;
    detailImage: string;
    originalImage: string;
    additionalMedia: string;
    addMediaLink: string;
    mediaUrl: string;
    contentType: string;
    mediaName: string;
    noMediaFound: string;
    confirmDeleteMedia: string;
    mediaUpdatedSuccess: string;
    mediaAddedSuccess: string;
    mediaDeletedSuccess: string;
    configItemId: string;
    configItemName: string;
    configType: string;
    optionsCount: string;
    createConfigItem: string;
    editConfigItem: string;
    configItemDetail: string;
    noConfigItemsFound: string;
    options: string;
    addOption: string;
    optionName: string;
    confirmDeleteConfigItem: string;
    confirmDeleteConfigOption: string;
    configItemCreatedSuccess: string;
    configItemUpdatedSuccess: string;
    configItemDeletedSuccess: string;
    configOptionCreatedSuccess: string;
    configOptionDeletedSuccess: string;
    searchInventoryPlaceholder: string;
    searchTransfersPlaceholder: string;
    searchLocationsPlaceholder: string;
    searchConfigPlaceholder: string;
    picklistsTab: string;
    lotsTab: string;
    createPicklist: string;
    createPicklistDesc: string;
    picklistDetail: string;
    pickableOrders: string;
    noPickableOrders: string;
    pickItems: string;
    binsAndOrders: string;
    statusHistory: string;
    printSheet: string;
    pickSheet: string;
    markAssigned: string;
    markPrinted: string;
    markAllPicked: string;
    totalItems: string;
    pickedItems: string;
    pendingItems: string;
    binsCount: string;
    noItemsInPicklist: string;
    noBinsDefined: string;
    noHistory: string;
    itemPickedSuccess: string;
    statusUpdated: string;
    createLot: string;
    createLotDesc: string;
    lotId: string;
    expirationDate: string;
    initialQuantity: string;
    saveLot: string;
    totalLots: string;
    expiringLots: string;
    noLotsFound: string;
    searchLotsPlaceholder: string;
    searchPicklistsPlaceholder: string;
    inventoryItemHistory: string;
    inventoryHistoryDesc: string;
    noHistoryRows: string;
    editTracking: string;
    serialNumber: string;
    locationSeqId: string;
    actions: string;
    facilityRequired: string;
    shipmentMethod: string;
    description: string;
    orderId: string;
    customer: string;
    itemCount: string;
    amount: string;
    physicalInventoryTab: string;
    facilitiesTab: string;
    directReceiveBtn: string;
    physicalCountBtn: string;
    newFacilityBtn: string;
    searchPhysicalPlaceholder: string;
    searchFacilitiesPlaceholder: string;
    allVarianceReasons: string;
    allFacilityTypes: string;
    noPhysicalInventoriesFound: string;
    noFacilitiesFound: string;
    sheetId: string;
    stockDiff: string;
    locationCount: string;
    inventoryCount: string;
    facilitySize: string;
    editFacility: string;
    physicalInventoryTitle: string;
    physicalInventoryDesc: string;
    systemQoh: string;
    countVariance: string;
    countedQuantity: string;
    countedQtyHelp: string;
    saveAdjustment: string;
    directReceiveTitle: string;
    directReceiveDesc: string;
    receiveInventoryBtn: string;
    directReceiveNotePlaceholder: string;
    productAndFacilityRequired: string;
    invalidQuantity: string;
    invalidCountedQty: string;
    newFacility: string;
    facilityFormSubtitle: string;
    createFacilityBtn: string;
    facilityNameRequired: string;
    facilityType: string;
    facilityDescPlaceholder: string;
    facilityManagementTitle: string;
    directReceiveStock: string;
    physicalCountAdjustment: string;
    newPhysicalCount: string;
    createFacility: string;
    totalCounts: string;
    negativeVariances: string;
    positiveVariances: string;
    netVarianceQty: string;
    totalFacilities: string;
    totalLocationsCount: string;
    totalActiveStockItems: string;
    searchPhysicalInventoryPlaceholder: string;
    searchFacilityPlaceholder: string;
    countDate: string;
    product: string;
    facilityName: string;
    facilityId: string;
    ownerPartyId: string;
    comments: string;
    commentsPlaceholder: string;
    noLocation: string;
    quantityAccepted: string;
    quickAdjustment: string;
    physicalCountSuccess: string;
    directReceiveSuccess: string;
    facilityCreatedSuccess: string;
    facilityUpdatedSuccess: string;
    qohVar: string;
    atpVar: string;
  };
  orders: {
    title: string;
    allOrders: string;
    salesOrders: string;
    purchaseOrders: string;
    orderId: string;
    orderName: string;
    orderType: string;
    customerOrVendor: string;
    orderDate: string;
    totalAmount: string;
    status: string;
    itemCount: string;
    items: string;
    adjustments: string;
    roles: string;
    statusHistory: string;
    createNewOrder: string;
    orderDetail: string;
    changeStatus: string;
    confirmApproveOrder: string;
    confirmCompleteOrder: string;
    confirmCancelOrder: string;
    confirmHoldOrder: string;
    statusUpdatedSuccess: string;
    orderCreatedSuccess: string;
    searchOrdersPlaceholder: string;
    noOrdersFound: string;
    totalOrdersCount: string;
    pendingOrders: string;
    approvedOrders: string;
    completedOrders: string;
    kpiPending: string;
    kpiApproved: string;
    kpiCompleted: string;
    kpiTotal: string;
    product: string;
    quantity: string;
    unitPrice: string;
    lineTotal: string;
    type: string;
    amount: string;
    role: string;
    party: string;
    user: string;
    reason: string;
    changeReasonPlaceholder: string;
    approveOrder: string;
    completeOrder: string;
    cancelOrder: string;
    holdOrder: string;
    originFacility: string;
    productStore: string;
    createdBy: string;
    salesOrder: string;
    purchaseOrder: string;
    currency: string;
    selectParty: string;
    selectProduct: string;
    addItem: string;
    removeItem: string;
    shippingAmount: string;
    taxAmount: string;
    subtotal: string;
    grandTotal: string;
    initialStatus: string;
    atLeastOneItem: string;
    partyRequired: string;
    saveOrder: string;
    quotesTab: string;
    returnsTab: string;
    ordersTab: string;
    quoteId: string;
    quoteName: string;
    quoteType: string;
    issueDate: string;
    validThru: string;
    convertToOrder: string;
    confirmConvertToOrder: string;
    quoteConvertedSuccess: string;
    createNewQuote: string;
    quoteDetail: string;
    returnId: string;
    returnType: string;
    returnReason: string;
    createNewReturn: string;
    returnDetail: string;
    createInvoiceQuick: string;
    createShipmentQuick: string;
    invoiceCreatedSuccess: string;
    shipmentCreatedSuccess: string;
    confirmCreateInvoice: string;
    confirmCreateShipment: string;
    returns: string;
    quotes: string;
    searchQuotesPlaceholder: string;
    noQuotesFound: string;
    searchReturnsPlaceholder: string;
    noReturnsFound: string;
    customerReturn: string;
    vendorReturn: string;
    returnHeaderType: string;
    returnPrice: string;
    returnQty: string;
    orderIdOptional: string;
    acceptReturn: string;
    completeReturn: string;
    cancelReturn: string;
    confirmAcceptReturn: string;
    confirmCompleteReturn: string;
    confirmCancelReturn: string;
    returnCreatedSuccess: string;
    saveReturn: string;
    partyId: string;
  };
  shipments: {
    title: string;
    subtitle: string;
    allShipments: string;
    newShipment: string;
    tabAll: string;
    tabSales: string;
    tabPurchase: string;
    tabInTransit: string;
    kpiTotal: string;
    kpiSales: string;
    kpiPurchase: string;
    kpiInTransit: string;
    kpiDelivered: string;
    searchPlaceholder: string;
    showingResults: string;
    records: string;
    shipmentId: string;
    type: string;
    orderId: string;
    route: string;
    carrierTracking: string;
    shipDate: string;
    status: string;
    action: string;
    noShipmentsFound: string;
    shipmentDetailTitle: string;
    createdDate: string;
    changeStatusTo: string;
    markPacked: string;
    markShipped: string;
    markDelivered: string;
    cancelShipment: string;
    markReceived: string;
    parties: string;
    partyFrom: string;
    partyTo: string;
    warehouses: string;
    originFacility: string;
    destFacility: string;
    linkedOrder: string;
    carrierAndTracking: string;
    carrier: string;
    trackingNumber: string;
    noTracking: string;
    shipmentItems: string;
    productId: string;
    productName: string;
    quantity: string;
    noItemsInShipment: string;
    shipmentReceipts: string;
    receiptId: string;
    product: string;
    acceptedQty: string;
    rejectedQty: string;
    receiptDate: string;
    confirmStatusChange: string;
    createShipmentTitle: string;
    createShipmentSubtitle: string;
    shipmentType: string;
    salesShipment: string;
    purchaseShipment: string;
    transferShipment: string;
    linkedOrderId: string;
    estimatedShipDate: string;
    estimatedArrivalDate: string;
    handlingInstructions: string;
    addItem: string;
    productIdPlaceholder: string;
    addItemError: string;
    createShipmentBtn: string;
    receiveInventoryTitle: string;
    receiveInventorySubtitle: string;
    targetFacility: string;
    receiptComments: string;
    receiptCommentsPlaceholder: string;
    itemsToReceive: string;
    fillRemaining: string;
    itemSeq: string;
    orderedQty: string;
    receivedQty: string;
    remainingQty: string;
    acceptQty: string;
    rejectQty: string;
    confirmReceipt: string;
    selectFacilityError: string;
    noItemsToReceiveError: string;
    linkedDocuments: string;
    linkedInvoices: string;
    linkedShipments: string;
    noLinkedInvoices: string;
    noLinkedShipments: string;
    invoiceId: string;
    invoiceDate: string;
    invoiceTotal: string;
    viewInvoice: string;
    viewShipment: string;
    receiveGoodsBtn: string;
    fulfillShipBtn: string;
    fulfillShipConfirm: string;
  };
  manufacturing: {
    title: string;
    subtitle: string;
    totalRuns: string;
    createdCount: string;
    scheduledCount: string;
    runningCount: string;
    completedCount: string;
    tabProductionRuns: string;
    tabBoms: string;
    tabRoutings: string;
    newProductionRun: string;
    productionRunId: string;
    workEffortName: string;
    product: string;
    facility: string;
    quantity: string;
    produced: string;
    startDate: string;
    completionDate: string;
    status: string;
    actions: string;
    noProductionRuns: string;
    statusCreated: string;
    statusScheduled: string;
    statusPrinted: string;
    statusRunning: string;
    statusCompleted: string;
    statusClosed: string;
    statusCancelled: string;
    confirmRun: string;
    startRun: string;
    completeRun: string;
    cancelRun: string;
    declareProduce: string;
    createModalTitle: string;
    createModalSubtitle: string;
    selectProduct: string;
    selectFacility: string;
    selectRouting: string;
    runName: string;
    quantityToProduce: string;
    startDateLabel: string;
    descriptionLabel: string;
    createButton: string;
    detailTitle: string;
    detailSubtitle: string;
    lifecycleActions: string;
    tabTasks: string;
    tabComponents: string;
    tabDeclare: string;
    taskSequence: string;
    taskName: string;
    taskEstimatedTime: string;
    taskActualTime: string;
    componentCode: string;
    componentName: string;
    requiredQty: string;
    issuedQty: string;
    noTasks: string;
    noComponents: string;
    declareQuantity: string;
    declareLotId: string;
    declareButton: string;
    declareSuccess: string;
    bomTitle: string;
    bomSubtitle: string;
    selectFinishedGood: string;
    allFinishedGoods: string;
    addComponent: string;
    componentCount: string;
    scrapFactor: string;
    sequence: string;
    noBoms: string;
    addBomModalTitle: string;
    addBomModalSubtitle: string;
    deleteBomConfirm: string;
    routingTitle: string;
    routingSubtitle: string;
    addRouting: string;
    routingName: string;
    stepCount: string;
    noRoutings: string;
    createRoutingTitle: string;
    createRoutingSubtitle: string;
    allStatuses: string;
    allFacilities: string;
    showing: string;
  };
}
